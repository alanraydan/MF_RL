...
def learn_mean_field(n_episodes, run, rho_V, rho_pi, omega_init, outdir):
...
if __name__ == '__main__':
    runs = [0, 1, 2, 3, 4]
    n_episodes, rho_V, rho_pi, omega_init = get_params()
    outdir = f'{n_episodes}eps_{omega_init}omega0'
    Parallel(n_jobs=len(runs))(
        delayed(learn_mean_field)(n_episodes, run, rho_V, rho_pi, omega_init, outdir + f'_run{run}') for run in runs
    )

...
from utils import save_actor_critic
...
if __name__ == '__main__':
    ...
    search_space = list(itertools.product(learning_rates, learning_rates))
    for i, (actor_lr, critic_lr) in enumerate(search_space):
        print(f'Run {i + 1}/{len(search_space)}')
        print(f'{actor_lr =}, {critic_lr =}')
        outdir = f'{n_episodes}eps_actor{actor_lr}_critic{critic_lr}'
        Parallel(n_jobs=len(runs))(
            delayed(learn_control)(n_episodes, run, critic_lr, actor_lr, outdir + f'_run{run}') for run in runs
        )
